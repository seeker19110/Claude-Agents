...
Ca ở đây dựng một CÔNG TY GIẢ (`topics/` tạm trong tmp_path, hai topic, một `Envelope` con): core không được
biết tên topic của company hay của studio, nên test của core cũng không được mượn chúng. Bài học K3.3c2: mã ở
