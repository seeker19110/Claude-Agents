...
| [`xagents-core/`](xagents-core/) | Lõi chung của mọi công ty AI: bus, llm, runner, guard, gate — company và studio import từ đây thay vì mỗi bên một bản fork | mypy `strict` + phủ 100% dòng từ ngày đầu, 162 test; ADR gốc 0001, 0003, 0004 (0002 đặt trước cho K4); đang xây theo bảy bước K3.1–K3.7 — **K3.3 xong hết**: cả bốn adapter lên chung, `ClaudeCodeClient` dưới dạng lớp cơ sở chỉ có transport (chính sách tool ở lại mỗi công ty), và K3.3d đưa nốt `routing.py` lên core — studio nay hoãn `TransientError` thay vì tính lỗi agent. Tiếp theo là K3.4–K3.7 |
