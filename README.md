...
| [`xagents-core/`](xagents-core/) | Lõi chung của mọi công ty AI: bus, llm, runner, guard, gate — company và studio import từ đây thay vì mỗi bên một bản fork | mypy `strict` + phủ 100% dòng từ ngày đầu, 130 test; ADR gốc 0001, 0003, 0004 (0002 đặt trước cho K4); đang xây theo bảy bước K3.1–K3.7 — K3.3c2 đã đưa `AnthropicClient`/`CodexClient`/`FakeClient` lên chung, còn `OpenAICompatClient`/`ClaudeCodeClient` (K3.3c3) |
