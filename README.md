...
| [`console/`](console/) | Trực ban hợp nhất: một trang web cục bộ nhìn cả hai công ty — hàng đợi human gate, ticket, dây chuyền video, token và chi phí, gói tài khoản đang xoay — duyệt gate ngay tại chỗ khi bật `--allow-decide`, đổi model/backend từng công ty khi bật `--allow-config`, giao việc mới (yêu cầu phần mềm, brief kênh video) bằng form khi bật `--allow-submit`. Cập nhật tức thì bằng SSE, địa chỉ deep-link tới từng gate/ticket, tìm và lọc mọi bảng, cài được thành app (PWA). Đọc bus SQLite ở chế độ chỉ đọc; quyết định đi qua đúng `HumanGate`, việc mới đi qua đúng bus + schema của từng công ty | `127.0.0.1:8200`, chỉ thư viện chuẩn (`http.server`), 7 màn hình, chỉ đọc mặc định + token mỗi lần chạy, ADR 0001–0002 |
