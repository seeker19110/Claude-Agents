...
| K6.1 | **chờ K3** | | `xagents-core` chưa tồn tại; `strict = true` từ ngày đầu là mục của K3.0, không làm trước được |
...
| K7.1, K7.2, K7.5, K7.6 | chưa | | tách `index.html` thành ES module, `/api/boot`, `API.md` rút gọn, màn xưởng video (sau K7.1) |
...
| K8.2, K8.5 | chưa | | `make llm` mặc định một tài khoản; "ngày đầu của người thứ hai" (K8.5 là điều kiện của K9.3) |
