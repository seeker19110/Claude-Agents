...
| K3.3c2–d | chưa | | c2: `AnthropicClient`/`CodexClient`/`FakeClient` (đo được 0.90/0.93/0.96 — company là bản trên, studio nhận `timeout`, `TransientError`, `check_argv`, prompt qua stdin); c3: `ClaudeCodeClient` 0.32 + `OpenAICompatClient` 0.28 (lệch nhất, mỗi cái một quyết định hợp nhất riêng); d: `routing.py` + studio hoãn `TransientError` thay vì dừng |
| K3.4–K3.7 | chưa | | K3.6 đợi ≥ 5 ngày lịch sau K3.5 (K3.e) |
