...
| K3.1b | **chưa** | | studio dùng `fit` ("nghiệm thu riêng" của K3.1). Tách PR riêng vì studio **không có** `max_input_chars` nào cả (`grep` rỗng): đây là thêm cấu hình + đổi hành vi studio, không phải chuyển mã — trộn vào PR chuyển mã là phá bất biến "mỗi PR tự đứng" |
