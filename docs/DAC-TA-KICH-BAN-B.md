...
| K3.3b | **xong** | | `LLMConfig`/`load_config` lên core; hai công ty là **lớp con** (`PREFIX` là `ClassVar` = `CORE.prefix`, ghi đè ba móc `apply_backend_yaml`/`apply_yaml`/`apply_env`). Studio KHÔNG còn trường riêng nào; company giữ đúng 14 trường studio không có (CLI/MCP, retry, giá, ngân sách, nợ, sandbox) — chúng là **dữ liệu có kiểu**, gói vào một `dict[str, Any]` chung là đánh đổi lấy `Any` đúng thứ `strict` sinh ra để giữ. Đây cũng là PR đầu tiên có `company/core.py` + `studio/core.py` (`CORE`), thứ mà K3.4–K3.7 dựng lên trên. Bộ lọc `<PREFIX>_LLM_BACKENDS` tách thành `select_backends` và gọi SAU `apply_env` (biến `LLM_PROVIDER` xoá `backends`, nên thứ tự là hành vi chứ không phải khẩu vị). `types-PyYAML` vào nhóm dev của core: core không có `--ignore-missing-imports` |
| K3.3c1 | **xong** | | `Completion` + đường bóc JSON lên core. **Quyết định hợp nhất, không phải chuyển mã** (`difflib` 0.23, thấp nhất trong 22 symbol): giữ cả nửa nghiêm của company (JSON hỏng GIỮA cấu trúc vẫn đỏ) lẫn nửa cứu được của studio (model kể chuyện trước rồi mới trả JSON), phân biệt bằng chỗ JSON bắt đầu. Bằng chứng: `evals --replay --strict` hai công ty, danh sách PASS/FAIL giống hệt bản trước. Studio nhận thêm `cache_write_tokens`, `tool_mode` |
| K3.3c2–d | **xong** | #173, #174, #175, #176 | c2: `AnthropicClient`/`CodexClient`/`FakeClient` (#173); c3: `ClaudeCodeClient` + `OpenAICompatClient` (#174, #175); d: `routing.py` + studio hoãn `TransientError` (#176) |
...
| K3.5c | **xong** | #187 | `sqlite_bus` lên xagents_core (`difflib` 0.442 — bước duy nhất của K3.5 thật sự "lấy bản company"); studio nhận khoá, `check_same_thread=False`, `latest()`, `_persist_only`, `__del__`; `BUSY_TIMEOUT_S` của studio thành hằng chung; `Lease`/`_alive` lên core. Thứ tự tham số `InMemoryBus.__init__` đổi thành `(cfg, enforce_owners)` vì MRO của lớp con hai cha |
| K3.6a | **xong** | #188 | `registry` lên xagents_core; K3.6 tách làm bốn bước (đo `difflib`: evals 0.556, registry 0.429, blackboard 0.092, runner 0.036). Hình dạng lệch mới: một bên là TẬP CON của bên kia. `spec_cls` giữ trường `tools` của studio; `check_owners=False` ở studio vì ba skill chưa có agent chủ quản (`content-policy`, `cost-estimation`, `finops`) — nợ có thật, có ca ghi lại |
| K3.6b | **xong** | #189 | `blackboard` lên xagents_core (hình dạng lệch giống K3.5b: một bên thiếu). `project_id`+`content` lên core `SharedContext` cùng nó — chúng là cơ chế của blackboard, không phải trường của miền; ca canh K3.5a đổi thành cấm THEO TỪNG LỚP chứ không nới lỏng. Studio nhận khoá, `rehydrate()`, `content` |
| K3.6c | **xong** | #191 | `evals` lên xagents_core. `difflib` gộp 0.556 giấu ba hình dạng lệch khác nhau: `RecordingClient` hợp nhất hai chiều, `_run_case` là miền (hook), `main`+`CaseResult` là CHÍNH SÁCH CỔNG nên ở lại từng công ty — `CaseResult` mang hai trường `broken_recording`/`errored` thay vì một cờ. Không sửa dòng test nào của hai công ty; giữ nguyên bốn seam monkeypatch |
| K3.6d1 | **xong** | #194 | khung runner (`RunnerError`/`RunResult`/`Generated`/`payload_schema`/`output_schema`) lên xagents_core. K3.6d tách đôi vì BẢN GHI EVAL: khoá bản ghi là hash(prompt), đo được là thêm một dấu cách vào prompt làm lệch toàn bộ — nên mọi chuỗi prompt giữ nguyên từng byte, nghiệm thu bằng `evals all --replay` hai công ty 0 FAIL |
| K3.6d2 | **xong** | #195 | `AgentRunner` (phần ngoài) lên xagents_core — **K3.6 xong**. Nguyên tắc: cơ chế lên core, hành vi mỗi công ty giữ nguyên từng byte. Hai chỗ đổi hành vi studio âm thầm (`wants_content`, `_new_envelope`) giải bằng tham số. Đo hai chiều để lộ lỗ hổng: đột biến làm core đỏ mà studio xanh → đã thêm ca chốt hành vi ở studio |
