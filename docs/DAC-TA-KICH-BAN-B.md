...
| K3.6b | **xong** | #PR | `blackboard` lên xagents_core (hình dạng lệch giống K3.5b: một bên thiếu). `project_id`+`content` lên core `SharedContext` cùng nó — chúng là cơ chế của blackboard, không phải trường của miền; ca canh K3.5a đổi thành cấm THEO TỪNG LỚP chứ không nới lỏng. Studio nhận khoá, `rehydrate()`, `content` |
