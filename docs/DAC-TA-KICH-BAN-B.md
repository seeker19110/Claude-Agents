...
| K3.3b–d | chưa | | b: `LLMConfig`/`load_config` tham số hoá qua `CoreConfig`; c: bốn adapter + `Completion.json()` (quyết định hợp nhất, phải đo bằng eval replay); d: `routing.py` + studio hoãn `TransientError` thay vì dừng. K5 mở khoá sau d |
