...
| K1 | ADR gốc 0020: chữ ký bất đối xứng, xoay khoá, lộ trình HMAC | docs | pe2-ky | C3 | Quyết định dependency trước code | Viết ADR tốn một vòng | xong #339 |
| K2 | Ed25519 + `key_id` + `not_after`; registry chỉ chứa public key; HMAC chỉ cho contract ghim `legacy_hmac` | company | pe2-ky | C3 | Verifier hết ký giả được | Thêm dependency `cryptography` | xong #339 |
| N1 | ADR gốc 0021 và orchestrator: đăng ký RunSpec khi ticket có profile, giao `quality:accept` cho trusted driver, CLI `commit` | company | pe2-noi | C3 | Adapter có giá trị thật | Đụng runtime, rủi ro hồi quy | ◐ PR #340 (code xong, ADR-0021 Accepted) |
| N2 | `quality_floor`: gap R6 khi run có profile mà `quality:accept` chưa `succeeded` | company | pe2-noi | C3 | Release không lọt khi thiếu nghiệm thu | Thêm một nguồn bằng chứng | ◐ PR #340 |
| N3 | Console hiển thị trạng thái `quality:accept` và blocker | console | pe2-noi | C2 | Người trực thấy lý do chặn | Thêm một view | ◐ PR #340 |
| N4 | Ghi chú ADR-0018 "Accepted, đã nối runtime"; `CODEMAP.md` có dòng quality | docs | pe2-noi | C1 | Tài liệu khớp thực tế | — | ◐ PR #340 |
