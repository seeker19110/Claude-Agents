...
| N1 | ADR gốc 0021 và orchestrator: đăng ký RunSpec khi ticket có profile, giao `quality:accept` cho trusted driver, CLI `commit` | company | pe2-noi | C3 | Adapter có giá trị thật | Đụng runtime, rủi ro hồi quy | xong #340 |
...
| N5 | ADR core và code: `quality:accept` chạy lại được khi candidate sha đổi sau `SUCCEEDED` (attempt mới ràng sha mới, giữ lịch sử) | core/company | pe2-noi2 | C3 | Nghiệm thu lại không cần ký profile mới | Đổi máy trạng thái ADR-0017 | chờ người: duyệt ADR-0022 (Proposed) và trả lời 4 câu hỏi trong đó |
