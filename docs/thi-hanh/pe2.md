...
| K1 | ADR gốc 0020: chữ ký bất đối xứng, xoay khoá, lộ trình HMAC | docs | pe2-ky | C3 | Quyết định dependency trước code | Viết ADR tốn một vòng | chờ người: ADR-0020 Proposed — đồng ý thêm dependency `cryptography` (kéo `cffi`, `pycparser`; uv.lock hiện 0 dòng) |
| K2 | Ed25519 + `key_id` + `not_after`; registry chỉ chứa public key; HMAC chỉ cho contract ghim `legacy_hmac` | company | pe2-ky | C3 | Verifier hết ký giả được | Thêm dependency `cryptography` | chờ người: phụ thuộc quyết định dependency ở K1 |
