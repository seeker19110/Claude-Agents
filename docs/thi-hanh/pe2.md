...
| N6 | `BusApprovalLookup` (người ký gate `SPEC-*` là nguồn sự thật của Ready), `--quality-trust` cho CLI orchestrator, `commit --db`; bổ sung ADR-0021 | company | pe2-duyet | C2 | Profile có `delivery` nghiệm thu được thật | Lookup quét lại audit-log mỗi lần nộp | ◐ PR #343 |
