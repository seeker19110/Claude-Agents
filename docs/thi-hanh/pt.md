...
| `pt.4` | **Đo trước khi sửa**: `SubagentStart` có tồn tại ở bản Claude Code đang chạy không, **và hợp đồng output của nó là gì** (stdout thô? JSON?) — bằng chứng máy sinh | — | **H3** `hook-subagent` | C2 | Chặn đúng chỗ luật bắt buộc 6 hay bị bỏ qua | Một gói không sinh code | **chờ người**: không đo được trong phiên thi hành từ xa (xem mục E) |
