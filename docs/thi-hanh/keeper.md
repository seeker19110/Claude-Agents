...
Ngày lập: 2026-09-09 · căn cứ `main@1d49da2` (#206) · đặc tả nguồn: `companies/keeper/docs/DAC-TA-KEEPER.md` (BT0 xong ở #207).

File này là **bảng theo dõi duy nhất** của việc thi hành `keeper`. Đặc tả `companies/keeper/docs/DAC-TA-KEEPER.md` giữ vai
...
1. `companies/keeper/` có đúng hai file tài liệu, **chưa một dòng mã**; BT1–BT8 còn nguyên.
...
|---|---|---|---|---|
| 1 | `CoreConfig(prefix, root, db_name, topic_acl, payload_models, namespace_owners)` | có, 12 trường, 3 bắt buộc; `env_name()` ghép `prefix` | `platform/xagents-core/src/xagents_core/config.py:29-96` | BT1 |
...
| 4 | adapter `gh` chỉ đọc, có timeout, không ném | **đã có bản đúng khuôn**: `encoding="utf-8"`, `env=clean_env()`, bắt `FileNotFoundError`/`TimeoutExpired`, cắt stderr `[-300:]` | `companies/software-company/src/company/github_pr.py:62` | BT2 |
| 5 | worktree riêng cho ticket, dọn được | **đã có**: `create()` idempotent, `remove(delete_branch)`, `fresh()`, `exclude_worktrees()` ghi `.git/info/exclude` chứ không `.gitignore` khách | `companies/software-company/src/company/workspace.py:86,103,114,178,62` | BT5 |
| 6 | "9 schema **sinh từ pydantic bằng script**, như hai công ty kia" | ✗ **không có script nào** sinh `topics/schemas/*.json`; 19 file của studio là artefact commit tay, ép khớp bằng test `set(get_args(Topic)) == set(bus._schemas)` | `Studio-creators/tests/test_bus.py:46`; `platform/xagents-core/src/xagents_core/bus.py:73` | BT1 |
| 7 | `SqliteBus` | ✗ tên thật là **`SQLiteBus`** | `platform/xagents-core/src/xagents_core/sqlite_bus.py:57` | BT7 |
...
| 9 | BT4 "dùng lại cơ chế `debt_due` của company, đọc `companies/software-company/src/company/orch/`" | ✗ sai địa chỉ **và** sai cơ chế: `debt_due` ở lõi, và tính quá hạn theo **chuỗi review liên tiếp** (`streak`/`fired`), **không theo ngày**. `DebtEntry(due_at=…)` là cơ chế MỚI | `platform/xagents-core/src/xagents_core/supervisor.py:82,102-123`; tiêu thụ ở `companies/software-company/src/company/orch/gates_flow.py:88` | BT4 |
| 10 | BT7 cạm bẫy "`gate_cli approve` là reopen" | ✗ `decide()` chỉ đóng gate và đẩy vào `history`; reopen là nghĩa riêng của một số escalation do orchestrator quyết theo loại subject | `platform/xagents-core/src/xagents_core/gates.py:87-94`; `orch/gates_flow.py:110-117` | BT7 |
...
| 14 | keeper có `agents/` → `assetscan` phải thấy | ✗ đường dẫn `. ../Studio-creators` hard-code ở **bốn chỗ** | `companies/software-company/Makefile:33,35`; `.github/workflows/ci.yml:412,415` | BT7 |
| 15 | tab console cho công ty thứ ba | ✗ tên công ty hard-code ở **5 chỗ backend** + `VIEWS`/`TITLES`/`SC,ST` frontend; `collect()`/`decide()`/`submit()` nhận đúng hai `Path` vị trí | `platform/console/src/console/collect.py:55`, `decide.py:22`, `brief.py:21`, `settings.py:25`, `server.py:40`; `static/js/router.js:26`, `state.js:9` | BT8 |
| 16 | test hợp đồng schema console | ✅ có, thêm công ty = thêm khoá `PACKAGES` | `platform/console/tests/test_hop_dong_schema.py:24` | BT8 |
| 17 | 8 system prompt + eval recordings | khuôn front matter có; `model_tier` là `str` tự do, ba giá trị thực `strong/standard/light` | `companies/software-company/agents/engineering/builder.md:1-24`; `companies/software-company/src/company/subagents.py:40` | BT7 |
| 18 | golden test cho agent | ✅ khuôn `UPDATE_GOLDEN=1 pytest tests/test_golden_agents.py` ở cả hai công ty | `companies/software-company/Makefile:22`; `tests/test_golden_agents.py:38,43` | BT7 |
| 19 | checklist gate `keeper` | khuôn mục gate: `Code gửi kèm:` → khoá → `Người tự kiểm thêm:` → `Kết quả:`; là nguồn sinh `sc-gate-*` | `companies/software-company/gates/checklists.md:26-42` | BT7 |
...
| BT8 | tab console + `HUONG-DAN-VAN-HANH` + `TRUC-VA-DUNG-KHAN` + README; **canary** | console, docs | `feat(keeper)` | C2 (platform/console/docs) + `chờ người` (canary) | nghiệm thu thật | canary cần `gh auth` + người merge | **xong #223** (console + tài liệu); **canary: chờ người** — cần `gh auth login` và người merge PR bảo trì đầu tiên (I1 cấm keeper tự merge) |
...
- *Không* sửa `companies/software-company/src/` để dùng chung `github_pr.py`/`workspace.py`. Đặc tả §0 cấm; `keeper` **sao
...
**Rủi ro của chính file này**: nó và `companies/keeper/docs/DAC-TA-KEEPER.md` nói về cùng tám việc. Chống trôi bằng đúng một
...
Gói việc đầy đủ (7 mục) **đã nằm trong `companies/keeper/docs/DAC-TA-KEEPER.md`** — BT1 ở §3, BT2 §4, BT3 §5, BT4 §6,
...
- Bảng file của §3 thiếu, phải thêm vào cùng PR: `Makefile:3` (`MEMBERS` + `companies/keeper/Makefile` có đủ 5 target
  ...
  chạy `uv sync --locked`); `.gitignore` (`companies/keeper/keeper.sqlite*`, `companies/keeper/var/`); `.github/CODEOWNERS`;
...
- `companies/keeper/README.md` **không ghi số test** (có cổng tự kiểm số liệu README ở hai công ty; ghi số là tự chuốc một
...
- Câu "dùng lại cơ chế `debt_due` của company" sai (A2-9). Gói phải mở bằng một đoạn nói rõ: cơ chế lõi đo **chuỗi
  review liên tiếp**, `keeper` cần **ngày đáo hạn** — đây là cơ chế mới trong `companies/keeper/src/keeper/ledger.py`, không
...
- Thêm `"keeper": "keeper"` vào `PACKAGES` của `platform/console/tests/test_hop_dong_schema.py:24`.
