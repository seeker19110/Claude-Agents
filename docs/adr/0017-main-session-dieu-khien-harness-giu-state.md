...
4. Trạng thái vòng đầu:
   ...
   *Sửa đổi bởi ADR-0022:* task **sink** đã `SUCCEEDED` được `TASK_REOPENED` (bắt buộc có lý do) về `READY` khi
   candidate đổi; run `SUCCEEDED` khi đó về `RUNNING`. Task có task hạ nguồn vẫn không mở lại được.
