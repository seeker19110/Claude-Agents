...
| **Gate** (human gate) | Điểm dừng để **người** ký: company `spec` → `release` → `acceptance` (+ `escalation`); studio `plan` → `publish` → `replies`. Hạn 24 h, nhắc 12 h, four-eyes. | Gọi kiểm tra bằng code là "gate" — đó là **guard** hoặc kiểm FSM (`_check_plan`). |
