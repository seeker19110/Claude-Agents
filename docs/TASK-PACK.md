...
|---|---|---|
| 2026-09-13 | `branch = true` ở `companies/software-company` (phép A6) — phiên 2026-09-13 bù xong 16/67 nhánh (`gate_cli.py`, `orch/gates_flow.py`, `orch/rehydrate.py`, `probe.py`, `supervisor.py`; xác nhận bằng `uv run pytest -q -n auto --cov --cov-report=term-missing --cov-fail-under=0` với `branch = true` tạm bật). CÒN **51 nhánh / 12 file**: `orch/{release_fsm,routes,scheduler,ticket_fsm,verify,worktree_flow}.py`, `orchestrator.py`, `runner.py`, `subagents.py`, `tools.py`, `web.py`, `workspace.py` | Còn 12 file cần TDD từng nhánh (FSM release/ticket, `runner.py`/`tools.py`/`web.py` sâu) — tiếp tục nhiều PR nhỏ theo cụm file (đã tách theo `orch/`, `probe+supervisor` ở hai PR đầu), không phải một buổi |
