...
|---|---|---|
| 2026-09-13 | `branch = true` ở `companies/software-company` (phép A6) — đợt 1 (16/67 nhánh: `gate_cli.py`, `orch/gates_flow.py`, `orch/rehydrate.py`, `probe.py`, `supervisor.py`) + đợt 2 (48/51 nhánh còn lại: 11/12 file đạt 100% nhánh — `orch/{release_fsm,routes,ticket_fsm,verify,worktree_flow}.py`, `orchestrator.py` (2/3), `runner.py`, `subagents.py`, `tools.py`, `web.py`, `workspace.py`). CÒN ĐÚNG **3 nhánh bế tắc thật** (`orchestrator.py:332`, `orch/scheduler.py:56` và `:250`) — không phải thiếu test: cả ba có hình `with <lock>: \n if <cond>: return <x>` (return một dòng trong khối `with`), đo trực tiếp xác nhận coverage.py không bao giờ ghi nhận arc đó dù test gọi đúng nhánh và assert đúng hành vi (chi tiết đo ở `pyproject.toml` ngay trên dòng `branch = true` đang comment, và `docs/sessions/2026-09-13.md`) | Cần người quyết cách xử lý 3 nhánh bế tắc trước khi bật `branch = true` thật: viết lại `return` xuống dòng riêng (đổi code chỉ để lách công cụ đo), chấp nhận qua bằng `# pragma: no branch` có ghi chú, hay giữ nguyên tắt — không phải việc TDD thêm test giải quyết được |
| 2026-09-13 | ADR-0004 quyết định 2 (`lessons_for(ticket)` — lọc bài học theo `assignee`/`risk_tags`/`retry>0`, cắt ≤5 bản mới nhất, đưa vào prompt runner) chưa có trong mã dù K3.6b (blackboard vào core) đã xong; đã ghi từ `docs/reports/2026-09-07-tu-kiem.md:40`, tái xác nhận ở audit A4 2026-09-13 (`grep -rn lessons_for src` → 0 kết quả) | Không phải việc của audit (đọc-đối-chiếu, không viết code); cần người quyết làm tiếp hay đóng lại vì `supervisor.lessons()`/`calibration()` đã đủ dùng — nếu làm thì cần ADR/ticket riêng, đi đủ bảy bước `CONTRIBUTING.md` §3 nếu chạm `agents/supervisor.md` |
