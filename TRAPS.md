...
| **Hook cổng đọc `CLAUDE_PROJECT_DIR` thay vì cây đang commit** | 2026-09-15 (`pt.10`): `CLAUDE.md` luật 2 bắt mỗi phiên một worktree, nhưng `pre-commit-gate.sh` đọc `git -C "$CLAUDE_PROJECT_DIR"` = checkout chính. Hỏng **cả hai chiều cùng lúc**: phép 1 thấy nhánh `main` của checkout chính → chặn oan mọi commit đúng luật; phép 2–4 đọc index checkout chính (rỗng) → file cấm, hạ `fail_under` và cổng gói **không được canh gì cả**. Không ca test nào thấy vì mọi ca cũ đặt cwd = `CLAUDE_PROJECT_DIR` | Trong hook: tách hai nghĩa thành hai biến — `$ROOT` chỉ để **tìm script** trong repo, `$CAY = git rev-parse --show-toplevel` (lùi về `$ROOT` nếu rỗng) cho **mọi phép đọc trạng thái git**. Ca test của hook phải dựng worktree thật, nhánh của checkout chính khác nhánh worktree; hai chiều (không chặn oan + vẫn chặn file cấm). Đã rà cả họ: `block-dangerous-git.sh` không đọc trạng thái git (chỉ parse chuỗi lệnh) và `auto-format.sh` chỉ dùng `$ROOT` để tìm `dev-task.sh` + thao tác trên đường dẫn tuyệt đối từ payload → cả hai an toàn, không phải sửa |
