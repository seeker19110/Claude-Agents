...
- **Có runtime chạy được: `kind` khai rõ; ứng dụng có lệnh khởi động, cổng, đường health và phụ thuộc ngoài** (`spec.runtime`)
  - nguồn: approved-specs@latest theo project_id — payload.kind; payload.runtime.command/port/health/dependencies (ADR-0031; orchestrator đọc bằng `smoke.parse_runtime`)
  - nguồn: prd@latest — mục 8b `Chạy ở đâu`: lệnh, cổng, health, phụ thuộc ngoài khớp payload
  - nguồn: audit-log `spec.runtime_missing` theo project_id — số lần spec bị trả lại vì thiếu runtime
