...
| Tính "hết lượt" là "làm sai" | Đo 2026-09-24 (CAMPUS-UNI): builder `claude -p` `error_max_turns` 8 lần trên 5 ticket; TCK-011/015 cháy 2/3 retry vì hết lượt trong khi WIP vẫn được giữ, lần ba test đỏ → `blocked`, 32 ticket phụ thuộc đứng chờ gate `escalation`. Cùng ngày ops `error_max_structured_output_retries` trên REL-003 → gate ngay lần đầu | Hết lượt mà lượt đó CÓ sửa đổi (`ws.dirty()`) → làm tiếp, không tính retry, trần riêng `MAX_TURN_CONTINUATIONS` (`turn_continuations`, khuôn `conflict_retries`); lỗi ép schema của CLI → tự thử lại đúng một lần (`llm.autoretry`, khoá `once`). Đọc `subtype` từ `LLMError.head`, không từ chữ model (`tests/test_on_dinh_auto.py`). Câu hỏi kiểm: **bộ đếm retry này đang đếm lỗi của agent, hay đếm trục trặc của hạ tầng/trần?** |
