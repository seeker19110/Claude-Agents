...
| SBOM + license cho release-check của security (`evidence.supply_chain`, `sbom_ref`) | `src/company/supply_chain.py` (`components` đọc `uv.lock`, `installed_licenses`, `spdx_of`, `evidence`); hook `orch/verify.py::evidence_before`/`evidence_after` (chung với smoke QA hồi quy); ADR-0046 |
