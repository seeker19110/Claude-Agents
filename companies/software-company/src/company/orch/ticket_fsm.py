...
def _threat_model(o: Orchestrator, env: Envelope, sid: str, res: StepResult) -> bool:
    """Agent `qa` pha `security` (ADR-0040) đọc spec đã duyệt: threat model v1 lên blackboard + review-results key=SPEC-*.
    ...
    if prior is not None and prior.payload.get("verdict") != "block":
        return True
    try:
        g = o.runner.generate(ROLE.QA, env, "review-results", phase="security")
        p = {**g.payloads[0], "ticket_id": sid, "source": SOURCE.SECURITY}
        o.runner.publish(ROLE.QA, env, "review-results", p, key=sid, tokens=g.tokens, model=g.model,
                            context_writes=g.context_writes, generated=g)
        with o._lock: o.stats["runs"] += 1
    except TransientError as e:
        res.actions.append(f"transient:{ROLE.QA}:{str(e)[:120]}")
        with o._lock: o.stats["transient"] += 1
        ...
        res.actions.append(f"error:{ROLE.QA}:{str(e)[:120]}")
        with o._lock: o.stats["errors"] += 1
        return True
    if p["verdict"] == "block":
