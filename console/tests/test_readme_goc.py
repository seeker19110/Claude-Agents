...
COMPANY_ROW_RE = re.compile(
    r"\[`software-company/`\].*?7 khối, (\d+) agent.*?ADR (\d{4})–(\d{4})",
)
...
def test_so_agent_khop_dia() -> None:
    agent_files = list((ROOT / "software-company" / "agents").glob("*/*.md"))
    match = _company_row()
    assert int(match.group(1)) == len(agent_files), (
        f"README gốc ghi {match.group(1)} agent nhưng software-company/agents/*/*.md có {len(agent_files)} file"
    )


def test_so_adr_khop_dia() -> None:
    adr_files = list((ROOT / "software-company" / "docs" / "adr").glob("*.md"))
    match = _company_row()
    ...
    assert adr_cuoi - adr_dau + 1 == len(adr_files), (
        ...
        f"({adr_cuoi - adr_dau + 1} bản) nhưng software-company/docs/adr/*.md có {len(adr_files)} file"
    )
