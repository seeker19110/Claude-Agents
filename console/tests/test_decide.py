...
def test_duyet_gate_that(company_db: Path, studio_db: Path) -> None:
    ...
    try:
        ...
        bus.close()
    assert "REL-001" not in {g["id"] for g in collect(company_db, studio_db, gateway_url=DEAD_GATEWAY)["gates"]}
...
def test_four_eyes_chan_nguoi_tao(company_db: Path, studio_db: Path) -> None:
    ...
    assert "REL-001" in {g["id"] for g in collect(company_db, studio_db, gateway_url=DEAD_GATEWAY)["gates"]}
...
def test_xuong_la(company_db: Path, studio_db: Path) -> None:
    with pytest.raises(ValueError, match="xưởng lạ"):
        decide(company_db, studio_db, subject_id="REL-001", xuong="phòng-marketing", decision="approve",
               by="human:pm", reason="")
...
def test_verb_quyet_dinh_la(company_db: Path, studio_db: Path) -> None:
    for verb in ("merge", "pending", ""):
        with pytest.raises(ValueError, match="quyết định lạ"):
            decide(company_db, studio_db, subject_id="REL-001", xuong=COMPANY, decision=verb, by="human:pm", reason="")


def test_thieu_db(tmp_path: Path, studio_db: Path) -> None:
    with pytest.raises(GateError, match="chưa có file DB"):
        decide(tmp_path / "khong-co.sqlite", studio_db, subject_id="REL-001", xuong=COMPANY, decision="approve",
               by="human:pm", reason="")
    with pytest.raises(GateError, match="chưa có file DB"):
        decide(None, studio_db, subject_id="REL-001", xuong=COMPANY, decision="approve", by="human:pm", reason="")
...
def test_thieu_nguoi_duyet(company_db: Path, studio_db: Path) -> None:
    with pytest.raises(ValueError, match="người duyệt"):
        decide(company_db, studio_db, subject_id="REL-001", xuong=COMPANY, decision="approve", by="  ", reason="")
