...
Mô phỏng một công ty gia công phần mềm bằng hệ đa agent event-driven: 7 khối, 6 agent (5 công đoạn + supervisor),
...
tests/         pytest 979 ca / 61 file (bus, registry↔events, delivery+gates, supervisor, orchestrator, release flow, nhánh
               ...
               guard/blackboard, schema consistency, golden 6 agent + 5 hồ sơ gate, bộ sinh subagent, hồ sơ gate, rà soát bảo mật);
...
- Test: 979 ca pytest gồm golden 6 agent (`tests/golden/`), runner với client giả, bus SQLite, gate, worktree, tool boundary,
