...
Mô phỏng một công ty gia công phần mềm bằng hệ đa agent event-driven: 7 khối, 12 agent,
...
src/company/   events, bus, sqlite_bus, registry, delivery, supervisor, gates, gate_cli, blackboard (artifact store),
               ...
               subagents (sinh 16 trợ lý kiểm duyệt chỉ-đọc `.claude/agents/sc-*.md` từ agents/ + gates/checklists.md — `make subagents`),
...
evals/         ca eval prompt theo agent (YAML) — đủ 12 agent, mỗi agent ≥ 2 ca; recordings/ = phản hồi model đã ghi
tests/         pytest 1011 ca / 60 file (bus, registry↔events, delivery+gates, supervisor, orchestrator, release flow, nhánh
               ...
               guard/blackboard, schema consistency, golden 12 agent + 5 hồ sơ gate, bộ sinh subagent, hồ sơ gate, rà soát bảo mật);
...
- Test: 1011 ca pytest gồm golden 12 agent (`tests/golden/`), runner với client giả, bus SQLite, gate, worktree, tool boundary,
...
- **Trợ lý kiểm duyệt có hồ sơ bằng chứng** (`docs/dac-ta-tro-ly-kiem-duyet.md`, đủ 6 PR): 16 subagent Claude Code chỉ đọc
...
### Bước tiếp theo
1. Chạy `make eval-record AGENT=<id>` cho 12 agent với model thật, commit bản ghi để CI eval có răng.
