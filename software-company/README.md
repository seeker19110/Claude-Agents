...
Mô phỏng một công ty gia công phần mềm bằng hệ đa agent event-driven: 7 khối, 17 agent,
...
src/company/   events, bus, sqlite_bus, registry, delivery, supervisor, gates, gate_cli, blackboard (artifact store),
               ...
               subagents (sinh 21 trợ lý kiểm duyệt chỉ-đọc `.claude/agents/sc-*.md` từ agents/ + gates/checklists.md — `make subagents`),
...
evals/         ca eval prompt theo agent (YAML) — đủ 17 agent, mỗi agent ≥ 2 ca; recordings/ = phản hồi model đã ghi
tests/         pytest 1011 ca / 60 file (bus, registry↔events, delivery+gates, supervisor, orchestrator, release flow, nhánh
               ...
               guard/blackboard, schema consistency, golden 17 agent + 5 hồ sơ gate, bộ sinh subagent, hồ sơ gate, rà soát bảo mật);
...
- Test: 1011 ca pytest gồm golden 17 agent (`tests/golden/`), runner với client giả, bus SQLite, gate, worktree, tool boundary,
...
### Bước tiếp theo
1. Chạy `make eval-record AGENT=<id>` cho 17 agent với model thật, commit bản ghi để CI eval có răng.
