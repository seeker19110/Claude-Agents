...
def run_smoke(root: Path, rt: Runtime) -> dict[str, Any]:
    ...
    tiến trình chết trước khi trả lời, đuôi stderr để người đọc hiểu vì sao."""
    ...
    t0 = time.monotonic()
    try:
        proc = subprocess.Popen(argv, cwd=str(root), env=clean_env(), stdout=subprocess.DEVNULL,
    ...
    try:
        ...
        try:
            ...
            err = ""
        if err:
            out["stderr_tail"] = err[-STDERR_TAIL:]
    return out
