...
def _check_escalations(o) -> None:
    ...
    for tid in {*o.lead.blocked(), *o.paused}:
        ...
        if o.lead.state.get(tid) == "blocked" or n:
            o._remember(key)
            o.gate.request(GateRequest(kind="escalation", subject_id=tid, created_by="supervisor",
...
def _open_acceptance_gate(o, rid: str, res: StepResult) -> None:
    ...
    o.gate.request(GateRequest(kind="acceptance", subject_id=sid, created_by="account-manager",
...
def _stall(o, env: Envelope, agent: str, error: Exception, res: StepResult) -> bool:
    ...
    if pid not in o.gate.pending:
        o.gate.request(GateRequest(kind="escalation", subject_id=pid, created_by="supervisor",
...
def _record_lessons(o, rid: str) -> None:
    ...
    for tid in o.lead.release_tickets.get(rid, []):
        ...
        o.blackboard.write("supervisor", "knowledge", f"audit-log:lesson:{tid}", json.dumps(lesson, ensure_ascii=False))
